      ...
      >
        <Link
          ...
          className="text-sm font-medium text-foreground/60 hover:text-foreground
             py-1.5 flex items-center justify-between w-full group transition-colors"
        >
...
const ProjectItem = ({ data }: { data: Doc<"projects"> }) => {
  return (
    ...
    >
      ...
      <span className="text-xs text-foreground group-hover:text-foreground/60 transition-colors">
        {formatTimestamp(data.updatedAt)}
      </span>
    </Link>
  );
      ...
      {rest.length > 0 && (
        <div className="flex flex-col gap-2">
          <div className="flex items-center justify-between gap-2">
            ...
            >
              <span>View all</span>
              <Kbd className="bg-accent border text-[11px]">⌘K</Kbd>
            </button>
          </div>
